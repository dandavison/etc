...
# ///
"""
Abstract graph representation eval runner.

Same methodology as the architecture eval, but with meaningless node labels
(A, B, C, …) to isolate format comprehension from semantic reasoning.
"""
...
GRADER_SYSTEM = """\
You are a precise grader. You will receive a question about a directed graph, the ground-truth \
answer, and a model's response. Determine whether the model's answer to this specific question \
is correct.
...
async def run_all() -> dict:
    ...
    print(f"Running eval: {len(GRAPHS)} graphs x {len(formats)} formats x {RUNS_PER_CONDITION} runs")
    ...
    return aggregate(results, formats)
...
def print_report(benchmark: dict) -> None:
    ...
    print("=" * len(header))
    print("ABSTRACT GRAPH EVAL — PASS RATES (mean +/- stddev)")
