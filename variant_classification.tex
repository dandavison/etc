...
pathogenic. The available data is a matrix of $n$ individuals x $L$
genomic locations of SNP or indel variants, together with a column of
$n$ phenotype scores, and a pedigree (multiple disconnected pedigrees)
...
\p Myriad treat the data as containing the following independent
sources of information about VUS effects:

\begin{enumerate}
\item Occurrence of VUS \textit{in trans} with known deleterious variant
\item Variation in family disease incidence between individuals with and without the VUS 
\item Cosegregation of VUS with disease status in pedigrees
\item Sequence/transcript-based effect prediction
\item Phylogenetic conservation
\end{enumerate}


\section{Sources}

\paragraph{Easton et al. [Myriad] (2007) Am. J. Hum. Genet. 81:873} Useful. Describes mathematical details for (1), (2) and (3).


\section{Questions}

\begin{itemize}
\item Should we collect data on numbers of non-affected relatives in
  addition to affected?
\end{itemize}


\p The main scenario is:
...
\p We may also sequence some affected individuals. This is equivalent to
a sequenced individual having an affected relative with relatedness
